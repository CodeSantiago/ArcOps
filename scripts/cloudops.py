...
Usage:
    ...
    arcops serve                            # Start persistent server
    arcops dashboard                        # Open web UI in browser
    arcops stop                             # Stop the server
...
def ensure_server():
    ...
    if port is not None:
        return port
    print("☁️  Starting ArcOps server (first load ~30s)...")
    port = find_free_port()
    ...
    for _ in range(60):
        ...
        if _port:
            return _port
        time.sleep(2)
    print("❌ Failed to start ArcOps server")
    sys.exit(1)
...
def show_help():
  ...
  USAGE
    ...
    arcops serve                  → Start persistent server in background
    arcops dashboard              → Open web UI in browser
    arcops stop                   → Stop the persistent server
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "serve":
        port = ensure_server()
        print(f"☁️  ArcOps server ready on port {port}")
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "dashboard":
        ...
        try:
            ...
            print("☁️  ArcOps Dashboard opened in new terminal window")
            ...
            try:
                ...
                print("⚠️  Could not open new terminal. Run manually:")
