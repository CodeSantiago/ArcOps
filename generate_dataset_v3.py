...
# ── CITY MAPPING (200 examples) ────────────────────────────────────────
# Model must learn that city names map to region codes
city_map = {
    "Virginia": "us-east-1", "Ohio": "us-east-2", "California": "us-west-1",
    "Oregon": "us-west-2", "Ireland": "eu-west-1", "Frankfurt": "eu-central-1",
    "London": "eu-west-2", "Singapore": "ap-southeast-1", "Sydney": "ap-southeast-2",
    "Tokyo": "ap-northeast-1", "Sao Paulo": "sa-east-1", "Canada": "ca-central-1",
}
city_examples = []
for city, region in city_map.items():
    for size in ["t3.micro", "t3.medium", "m5.large"]:
        prompt = f"Create a {size} server in {city}"
        ex = ec2(size, region)
        ex["messages"][1]["content"] = prompt
        city_examples.append(ex)
        prompt = f"Restart the database in {city}"
        ex = rds("prod-db", region)
        ex["messages"][1]["content"] = prompt
        city_examples.append(ex)
...
noise_rds = [
    ...
    "set owner to db-admins",
    "created_by the monitoring system",
]
