...
A payload matching all schema constraints SHALL pass validation.

#### Scenario: create_ec2_instance valid payload
- GIVEN the `create_ec2_instance` schema requiring `instance_type` (string), `ami_id` (string), `region` (string), `min_count` (integer), `max_count` (integer)
- WHEN a payload with all required fields and correct types is validated
- THEN the result MUST indicate success

Example well-formed payload:
...
An invalid payload MUST be rejected with a clear error message.

#### Scenario: Missing required field
- GIVEN the `create_ec2_instance` schema
- WHEN a payload omits `ami_id`
- THEN validation MUST fail with an error naming the missing field

#### Scenario: Wrong value type
- GIVEN the `create_ec2_instance` schema
- WHEN `min_count` is a string (`"one"`) instead of integer
- THEN validation MUST fail with a type-mismatch error

#### Scenario: Extra unknown field
- GIVEN the `create_ec2_instance` schema (disallowing additional properties)
- WHEN a payload includes an unrecognized field `"foo": "bar"`
