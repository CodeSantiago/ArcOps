...
# ── Known instance types (for validation) ──────────────────────────────
...
def validate_schema(tool_name: str, arguments: dict) -> SafetyResult:
    ...
    # Validate instance type value
    if tool_name == "create_ec2_instance":
        itype = arguments.get("instance_type", "")
        if itype and itype not in KNOWN_INSTANCE_TYPES:
            result.warnings.append(
                f"Unknown instance type '{itype}' — not in AWS catalog. "
